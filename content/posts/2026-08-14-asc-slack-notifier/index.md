...
Once I started, it turned out the webhook payload is surprisingly bare: a version state change event carries **only the resource UUID and the old and new states**. The payload alone cannot tell you which app or which version moved, which makes a shared channel useless the moment you have more than one app. So I added optional enrichment — give it an App Store Connect API key and it looks the resource up before posting, adding the app name, version and build number, plus an "Open in App Store Connect" button.

## Usage

When a webhook arrives, the flow is:

- Verify the `x-apple-signature` HMAC-SHA256 header against the raw request body, compared in constant time
- Build a Block Kit message with an emoji per state (`READY_FOR_REVIEW` 📝, `PENDING_APPLE_RELEASE` ⏳, `READY_FOR_DISTRIBUTION` ✅, `REJECTED` ❌, …)
- Event types Apple hasn't documented yet are still delivered with a generic key/value rendering — notifications are never silently dropped
- When Slack can't be reached the service returns `502`, so App Store Connect redelivers
...
Bug reports and requests — like how a particular event type should be rendered — are welcome via [GitHub Issues](https://github.com/ngs/asc-slack-notifier/issues) or pull requests.
